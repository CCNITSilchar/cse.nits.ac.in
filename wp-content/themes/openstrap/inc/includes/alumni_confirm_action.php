...
if(!isset($_POST['submit']))
	...
	//Performing Delete and Update Query
		//update
		for($i=0;$i<$count_update;$i++)
			{
			$query1='UPDATE alumni SET visible=1 where id=';
			$query1.=$idu[$i].' limit 1';
			$result1=mysqli_query($connection_new,$query1);
		if($result1 && mysqli_affected_rows($connection_new)) // returns number of affected rows (-ve if error in result)
		 {
		  //success
